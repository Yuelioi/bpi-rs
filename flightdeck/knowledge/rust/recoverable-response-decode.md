...
activation: action
read_when: "before changing any transport response decoding, response-model error, or raw-response fallback behavior."
recheck_when: "BpiError, TransportResponse, serde_json, bytes, or the response logging policy changes."
---
...
- 先轻量解析 envelope 元信息；`code != 0` 时不得要求 `data` 符合成功 payload 模型，应直接保留 code/message 并返回 `BpiError::Api`。只有 `code == 0` 才严格反序列化 `T`。
...
- 非零 API code 即使携带结构不兼容的 `data`，也返回对应 `BpiError::Api`；同样的不兼容结构放在成功响应中仍返回 `BpiError::ResponseDecode`。
